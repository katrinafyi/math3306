...
\begin{figure}[h]
  ...
  \begin{minted}[frame=single,framesep=8pt,fontsize=\footnotesize]{python}
def main():
  while True:
    if tape('#', '#', R):
...
\begin{tikzpicture}[shorten >=1pt,node distance=2.2cm,on grid,auto] 
  ...
  \tikzset{
   ...
   \path[->] (q0) edge node [swap]{$\#/\#/R$} (qhash1);
   \node[state] (qhash2) [right=of qhash1] {$+$};
   \path[->] (qhash1) edge node {$\emptyset/\emptyset/R$} (qhash2);

   % ZEROS

   \node[state] (qzero1) [below left=of q0]{$\rightarrow$};
   \path[->] (q0) edge node [swap]{$0/\emptyset/R$} (qzero1);
   \path[->] (qzero1) edge [loop left] node {$[01]/[01]/R$} (qzero1);


   \node[state] (qzerocheck) [below =of qzero1]{};
   \path[->] (qzero1) edge  node [swap]{$\#/\#/R$} (qzerocheck);

   \node[state] (scanright) [below right=of qzerocheck] {$\rightarrow$};
   \path[->] (qzerocheck) edge node [swap]{$0/\emptyset/R$} (scanright);

   \path[->] (scanright) edge [loop right] node [yshift=-0.2cm]{$[01]/[01]/R$} (scanright);

   \node[state] (beginshift) [below =of scanright] {$\ll$};
   \path[->] (scanright) edge node [swap]{$\emptyset/\emptyset/L$} (beginshift);

   \node[state] (ones) [below right=3cm of beginshift] {$\ll$};
   \path[->] (beginshift) edge node {$1/\emptyset/L$} (ones);
   \node[state] (zeros) [below left=3cm of beginshift] {$\ll$};
   \path[->] (beginshift) edge node [swap]{$0/\emptyset/L$} (zeros);

   \path[->] (ones) edge [bend left] node [swap]{$0/1/L$} (zeros);
   \path[->] (ones) edge [loop right] node {$1/1/L$} (ones);
   \path[->] (zeros) edge [bend left] node [swap]{$1/0/L$} (ones);
   \path[->] (zeros) edge [loop left] node {$0/0/L$} (zeros);


   \node[state] (endshift) [below right=3cm of zeros] {$\ll$};
   \path[->] (zeros) edge  node [swap]{$\emptyset/0/L$} (endshift);
   \path[->] (ones) edge  node {$\emptyset/1/L$} (endshift);
   % \path[->] (beginshift) edge[bend left=4cm] node
   % [swap]{$\emptyset/\emptyset/L$} (endshift);

   \draw [->,rounded corners=2.15cm](beginshift) to
   ($(beginshift)+(4.6, 0)$) to   node[xshift=0.1cm]{$\emptyset/\emptyset/L$}($(endshift)+(4.6,0)$) to  (endshift);

   \node[state] (hashagain) [below=of endshift] {$\leftarrow$};
   \path[->] (endshift) edge node [swap] {$\#/\#/L$} (hashagain);
   \path[->] (hashagain) edge [loop right] node  {$[01]/[01]/L$} (hashagain);
   ...
   ($(hashagain)+(-5, 0)$) to   node[swap,xshift=0.1cm]{$\emptyset/\emptyset/R$}($(q0)+(-5,0)$) to  (q0);
   ...
   \path[->] (q0) edge node {$1/\emptyset/R$} (qone1);
   \path[->] (qone1) edge [loop right] node {$[01]/[01]/R$} (qone1);
   \node[state] (qonecheck) [below =of qone1]{};
   \path[->] (qone1) edge  node {$\#/\#/R$} (qonecheck);
   \path[->] (qonecheck) edge  node {$1/\emptyset/R$} (scanright);
